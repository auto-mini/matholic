...
- 서버는 `0.0.0.0:48129`에 수신 대기하지만 페어링 QR에는 현재 선택된 사설
  ...
  같은 사설망인지 확인한다.
- 0.1.8부터 알림 영역에서 종료하거나 Tk 창의 mainloop가 끝나면 listener를 먼저
  닫고, 이미 연결된 socket을 종료한 뒤 handler 회수를 최대 2초 기다린다. handler가
  회수된 뒤에만 대기 CSV의 메모리 buffer를 지우므로 종료와 전송 상태 정리가
  서로 경쟁하지 않는다.
