...
## SOL-0036 관리자 화면에서 자동대기 복귀 불가 — 2026-08-13 17:58 +09:00

- 실제 A 화면을 직접 캡처·계층 대조했다. 목2 종료 뒤 `ADMIN_IDLE` 관리자 화면이
  열려 있었고 다음 예약은 금1 10:00이지만, 화면에는 다음 예약도 자동대기로 돌아가는
  제어도 없었다. 자동전환 정책은 `adminPanel`이 보이는 동안 의도적으로 전환을
  보류하므로 그대로 두면 다음날 자동 시작도 미뤄질 수 있었다.
- 보안상 자동 종료 자체는 PIN 화면으로 이동하고 관리자 panel은 PIN 인증 뒤에만
  열리며, 수동 시작 버튼을 누르면 오늘 자동전환 끄기 경고가 정확히 나오는 반대
  근거가 있다. 따라서 권한 우회·데이터 손실이 아닌 운영 가용성 P2로 판정했다.
- `7d5e07b`에서 상단 자동대기 버튼, 다음 예약·수동 시작 안내, 관리자 진입 scroll
  초기화와 수업 동선 상향을 반영했다. unit 111/111, focused 3/3, 전체 계측 87/87,
  lint와 signed release를 통과했다.
- A에 RC94/code 99를 보존 설치해 `다음 자동 시작: 금1 · 내일 10:00`을 직접
  확인했다. 새 버튼을 실제 누른 뒤 PIN 대기, 안내 문구, top Kiosk와 Lock Task
  `LOCKED`를 확인했다. 학생·반·QR·시간표는 바꾸지 않았고 원격 점검은 종료했다.
- RC94 SHA-256은
...
- 오늘 임시 시간표로 실제 종료 경계를 두 번 만들었다. QR 대기 시험은 15:35에
  자동 종료·`ADMIN_IDLE` 관리자 PIN 이동과 종료 안내를 통과했다.
- 승인 `테스트` 계정을 현재 목2 수업 보충으로만 임시 추가해 실제 25문항 문제 화면에
  진입했다. 15:45 종료 경계 뒤 15:45:27에도 Web 문제 1/25·답안 0/25가 유지돼
  채점 중 종료 보류를 확인했다. 답안은 입력하지 않았다.
- 15:46 `채점 종료·로그아웃` 뒤 15:46:35 Kiosk가 자동 종료·관리자 PIN 화면으로
  수렴했다. 오늘 임시 시간표를 삭제하고 주간 `1=10:00`, `2=13:00`으로 복원했으며,
  현재 예약 반 목2를 자동 시작해 QR 대기로 돌려놓았다.
- 최종 RC91 top, Lock Task `LOCKED`, 원격 지원 `INACTIVE`; 로컬 캡처,
  ADB forward/reverse와 test package는 없다. 이번 검증은 운영 source 변경 없이
  현장검증 공백을 닫았으며 이 기록 commit은 뒤에 생성한다.
