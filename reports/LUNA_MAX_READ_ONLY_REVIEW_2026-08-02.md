...
- RC60의 40×40mm QR을 유지하면서 기준 절단용 PDF와 같은 A4 3×3,
  55×80mm 카드, 가로·세로 5mm 간격과 0.25pt 절단선을 RC61에 적용했다.
- 합성 QR 10개 자동 재판독, 실제 2장 교정본의 QR image pixmap 동일성,
  release 158 tasks와 Android 13 계측 12개를 통과했다.
- 사용자가 실제 프린터 출력, 종이 절단과 절단 후 태블릿 카메라 QR 인식을
  수행했고 모두 **PASS**라고 확인했다. 따라서 채택된 리뷰 교정과 RC61 QR
  양식에는 남은 코드·현장 검증 항목이 없다.
...
- 교정 commit은 `a97d695`, 버전 정렬 commit은 `d1a4a08`이다. 당시 남았던
  실제 프린터 물리 출력은 위 RC61 절단용 교정본의 출력·절단·카메라 인식
  PASS로 후속 완료됐다.
...
- 사용자가 채택한 코드·문서 교정은 완료됐다. 당시 남았던 실제 프린터 종이
  출력도 위 RC61 현장 검증에서 절단·카메라 인식과 함께 PASS했다.
