...
학생 개인계정의 로그인·학생 확인·로그아웃을 보조하는 Android 앱의 단계별 검증 저장소다. Android 앱 Gate 1 실기 조사는 최종 FAIL이고, 공식 웹 경로의 Web Gate 2·3과 QR 운영 Gate 4 alpha는 PASS다. Gate 5에는 Device Owner, 전용 HOME, 두 앱 allowlist와 Lock Task 잠금이 구현됐다. A 기기(SM-P610)는 release signer 전용 Device Owner 기기다. RC02에서 QR→Web 문제 화면→로그아웃 자동 복귀, 비정상 Web 세션의 관리자 자체 복구, 잠금과 재부팅 복구, 120분 연속 운전, 실제 프린터 출력과 종이 QR 왕복을 통과했다. 현재 A 기기에는 기존 앱 데이터를 보존한 Kiosk RC39와 Web POC RC58이 설치되어 있다. 2026-07-30 RC58은 답안창 초기 렌더를 실패로 오인하지 않도록 1.5초 유예하고, 그 뒤에도 초기화되지 않은 개별 공식 입력기만 기존 답안을 보존한 채 기본→수식으로 재마운트한다. 별도 release signer는 Android 폰과 별도 USB에 암호화 키 복구본을 보관한다.
...
- `webpoc`: 화면 꺼짐을 막고 Lock Task allowlist 안에서 실행된다. 현재 소스와 A 설치본은 `0.4.0-rc58`이다. 학생 화면은 학습지·진단평가 전환만 남기고 Matholic 상단 메뉴, 문제지·오류신고·동영상·답안필기·풀이 업로드를 숨긴다. 학습 페이지 진입 때 공식 jQuery와 MathQuill을 순차 선로딩해 여러 주관식의 동시 초기화 경쟁을 막는다. 초기 공식 `span`은 1.5초 동안 정상 초기화를 기다리며, 이후에도 class 없는 개별 입력기만 공식 `onChange`로 기존 값을 유지한 채 기본→수식 전환해 재마운트한다. 이 과정에서 MathQuill ref, 루트·분수·파이, `userAnswers`와 임시저장 배열이 함께 복구된다. 표시 중인 편집기에만 최소 220×56px 터치 영역을 적용하고 겹친 회색 지우기 버튼을 숨긴다. 방향 키패드는 수식 입력란을 직접 터치하거나 포커스한 동안에만 표시하며 답안제출 등 다른 곳을 터치하면 즉시 숨긴다. 전체답안 내부 스크롤을 끝으로 이동하고 제출 버튼을 고정한다. 답안 제출 계열 버튼은 1.5초 동안 재진입을 막고 앱 확인창은 0.8초 뒤에만 누를 수 있다. 로그아웃·관리자 복구 중 렌더러 무응답 또는 시간 초과가 발생하면 기존 렌더러를 폐기하고 새 WebView로 한 번만 재시도한다. 기존 origin·경로·호출자·프록시·세션 실패폐쇄 경계는 유지한다.
- 외부 알림은 현재 요구사항에서 제외했다. release RC02는 정의된 운영 인수시험을 완료했고 Kiosk RC39/Web POC RC58 조합은 자동검증·보존형 설치까지 완료했다. RC58의 초기 렌더 무개입·지속 실패 재마운트·임시저장 배열 회귀시험은 통과했고 실제 학습지는 사용자 재확인 대기다.
