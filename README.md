...
- `kiosk`: Gate 4 기능과 Gate 5 Device Owner·전용 HOME·Lock Task를 제공한다. A 설치본은 `0.6.0-rc05`이며 관리자 비동기 단일 실행, QR 재발급 확인, 중복 반 이름 차단과 수업 저장소 불변조건을 포함한다. 현재 브랜치에는 재시작 뒤 늦게 도착한 Web 결과가 잠금을 풀지 못하게 하는 다음 검증본용 보강이 추가돼 있다.
