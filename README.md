...
학생 개인계정의 로그인·학생 확인·로그아웃을 보조하는 Android 앱의 단계별 검증 저장소다. Android 앱 Gate 1 실기 조사는 최종 FAIL이고, 공식 웹 경로의 Web Gate 2·3과 QR 운영 Gate 4 alpha는 PASS다. Gate 5에는 Device Owner, 전용 HOME, 두 앱 allowlist와 Lock Task 잠금이 구현됐다. A 기기(SM-P610)는 release signer 전용 Device Owner 기기다. RC02에서 QR→Web 문제 화면→로그아웃 자동 복귀, 비정상 Web 세션의 관리자 자체 복구, 잠금과 재부팅 복구, 120분 연속 운전, 실제 프린터 출력과 종이 QR 왕복을 통과했다. 현재 A 기기에는 기존 앱 데이터를 보존한 Kiosk RC94와 Web POC RC139가 설치되어 있다. 화면에 보이는 앱명·관리자 헤더·시험 모드명·보안 상태는 `채점 관리`와 `학습` 중심의 중립 문구로 바꾸고 QR 모양 앱 아이콘도 일반 문서 확인 아이콘으로 교체했다. 학습 화면에는 112×96px 문제 이동 버튼, 읽기 전용 현재 문제번호 배지, 답안 현황 지도, `모름` 안내와 답안 입력칸 아래쪽에 열리는 수식 키패드를 제공한다. QR 대기부터 학습지·진단평가 목록, 객관식·주관식 문제, 전체답안 확인과 결과까지 학생이 조작하는 각 화면에는 해당 화면용 큰 도움말을 여닫는 `?` 버튼을 제공한다. ACTIVE 학생 세션 동안 앱 창 밝기를 80%로 적용하고 로그아웃·복구·종료 때 이전 밝기로 복원한다. 별도 release signer는 Android 폰과 별도 USB에 암호화 키 복구본을 보관한다.
...
- `kiosk`: Gate 4 기능과 Gate 5 Device Owner·전용 HOME·Lock Task를 제공한다. 현재 소스는 `0.6.0-rc94`이다. 반별 시작 시각만 설정하면 3시간 수업을 자동 시작·전환·종료하고, 학생 채점·관리자 작업 중에는 QR 대기까지 전환을 보류하며, 오늘 임시 시간표와 오늘만 자동 끄기를 제공한다. 수업 종료 뒤 관리자 화면에는 다음 자동 시작과 지금 시작 절차를 표시하고, 상단 버튼으로 관리자 화면을 잠가 자동대기·Lock Task로 돌아간다. 전환 적용 직전 상태를 다시 읽어 오래된 판정으로 수업을 바꾸지 않으며, 같은 시간표 오류의 PC 알림은 10분 간격으로 제한한다. 시간표 버튼은 가려진 터치를 거부한다. 선택적 공유 QR PDF는 정상 복귀 뒤 정리하고, process 재시작 때 남은 수명을 다시 예약하며, 전용 FileProvider가 URI를 제공하기 전에 만료 파일을 정리한다. 첫 실행 때 신규용 더미 QR 4장을 `신규카드1`~`신규카드4`로 자동 준비해 지정 PC에 전송하며, 각 더미의 아이디·비밀번호는 빈 값으로 유지해 로그인되지 않게 한다. 자동 PDF 전송은 핵심 초기화와 별도 실행되므로 지정 PC가 응답하지 않아도 관리자 인증과 관리자 데이터 준비를 막지 않는다. 지정 PC가 TCP 연결 뒤 PDF를 읽지 않는 partial stall도 write timeout으로 끊어 전용 PC executor를 무기한 점유하지 않는다. 과거 비활성 재사용 슬롯이 있으면 같은 슬롯 라벨의 새 QR·빈 자격정보로 복구해 활성 카드 수를 4장으로 보충한다. 학생이 오면 기존 QR을 그대로 둔 채 이름·ID·PW만 더미 슬롯에 배정하고 현재 수업 보강 학생으로는 자동 추가하지 않는다. 앱 재시작, 이름·CSV 변경과 일반 QR 재발급 경로도 재사용 카드의 QR을 바꾸지 않는다. 저장 미확인 슬롯을 수동 교체할 때는 기존 QR 폐기와 이전 인쇄물 무효화를 먼저 경고한다. 이후 실제 QR 카드로 전환하면 새 일반 학생 행과 새 QR을 발급하고 반 소속을 옮긴 뒤, 기존 더미 슬롯은 빈 값으로 자동 초기화한다. 학생 CSV 가져오기는 PC 요청·parse·미리보기·적용 전체 수명 동안 공통 관리자 데이터 gate를 유지해 수업 시작과 다른 학생·반 작업이 겹치지 않게 한다. 신규·변경 QR 카드 PDF 생성, 카드 상태·이력, 관리자 30초 실행취소, 운영 자가진단·원버튼 복구, PC 상태·암호화 전송과 수식 키패드 배치 설정도 제공한다. 반 변경·보충 추가 성공 안내는 수동 거리·밝기 안내에 덮이지 않고 3초 뒤 자동으로 사라진다. 학생 QR 검증이 끝나면 지정 PC에 이름을 제외한 로그인 알림을 한 번만 요청한다. 실제 결과 확인 뒤 채점을 끝내면 PC 상태·알림에 오답 개수와 문제번호 요약을 표시한다. Android 직접 인쇄는 제거됐고 기존 반·학생·QR 데이터와 secure session 경계는 유지한다.
- `webpoc`: 화면 꺼짐을 막고 Lock Task allowlist 안에서 실행된다. 현재 소스는 `0.4.0-rc139`이다. secure renderer 복구를 위한 Activity 재생성은 백그라운드 잠금으로 오인하지 않는다. loopback CONNECT proxy는 동시 tunnel을 32개로 제한하고 성공 tunnel을 60초 idle 뒤 회수하며, upstream 연결 실패 시 할당한 socket을 즉시 닫는다. 네트워크 단절 안전 대기, 5분 무입력 경고와 1분 뒤 안전 로그아웃·QR 복귀, 미입력 문항 순회, 수식 입력 실행취소·다시실행, 삭제 길게 누르기, 답 입력칸 아래쪽 수식 키패드와 가변 터치영역을 제공한다. `채점 끝내기` 첫 터치에는 답안은 문제 화면의 `입력` 버튼으로 입력해야 한다는 강한 종료·로그아웃 경고를 표시한다. 사용자에게 노출되지 않는 제한된 진단 로그에는 중단 단계·느린 처리·자동 재시도·결과 판독 실패의 비식별 구조 정보만 남긴다. 기존 문제 번호·답안 현황·MathQuill 복구·제출 연타 차단·오답 추출과 실패폐쇄 경계는 유지한다.
- release RC02는 정의된 운영 인수시험을 완료했다. 현재 보관 검증 묶음과 실제 A 설치본은 Kiosk RC94/Web POC RC139이며 운영 PC는 수신기 0.1.8이다. A 보존 설치 뒤 UID·firstInstallTime·Device Owner·전용 HOME·Lock Task를 유지했다. RC94는 전체 Kiosk 계측 87/87, signed release, 다음 예약 표시와 관리자 자동대기 복귀 실기를 통과했다. RC139의 renderer 복구는 unit과 Android 13 에뮬레이터 강제 복구 1/1을 통과했지만 A에서 renderer 충돌을 고의 주입하지 않았다. CSV 요청 중 공통 gate는 지연 loopback PC 계측으로 확인했으며 운영 PC와 A에서 실제 CSV를 가져오지는 않았다. 실제 학생정보 입력·카드 배정·선택적 PDF 공유도 하지 않았다. PC 수신기 0.1.8은 active 연결 종료 barrier를 포함하며 운영 설치·독립 자체 점검과 TCP 48129 재기동을 통과했다. RC61 QR 카드의 실제 출력·절단·카메라 인식까지 통과했다.
