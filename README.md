...
학생 개인계정의 로그인·학생 확인·로그아웃을 보조하는 Android 앱의 단계별 검증 저장소다. Android 앱 Gate 1 실기 조사는 최종 FAIL이고, 공식 웹 경로의 Web Gate 2·3과 QR 운영 Gate 4 alpha는 PASS다. Gate 5에는 Device Owner, 전용 HOME, 두 앱 allowlist와 Lock Task 잠금이 구현됐다. A 기기(SM-P610)는 release signer 전용 Device Owner 기기다. RC02에서 QR→Web 문제 화면→로그아웃 자동 복귀, 비정상 Web 세션의 관리자 자체 복구, 잠금과 재부팅 복구, 120분 연속 운전, 실제 프린터 출력과 종이 QR 왕복을 통과했다. 현재는 기존 데이터와 Device Owner를 보존해 Kiosk RC29와 Web POC RC31을 설치했고 자동검증과 설치 후 무결성 검사를 통과했다. 2026-07-27 실물 회귀에서 기존 학생·반 보존, 반 소속 QR 차단, 관리자 뒤로가기 유지와 자동 학습지 진입은 통과했다. 후속 RC28/RC30은 학생 Web 상단 메뉴·불필요한 문제 기능을 숨기고, 수식 입력기·제출 화면·오답 전체 검증과 QR 위치 안내를 보강했다. RC29/RC31은 세로 전환 없이 두 가로 방향의 센서 회전을 허용한다. 별도 release signer는 Android 폰과 별도 USB에 암호화 키 복구본을 보관한다.
...
- `kiosk`: Gate 4 기능과 Gate 5 Device Owner·전용 HOME·Lock Task를 제공한다. 현재 소스와 A 설치본은 `0.6.0-rc29`이며 관리자 화면에서 시스템 뒤로가기를 소비해 현재 입력과 화면을 유지한다. 카메라 영상은 표시·저장하지 않고 매쓰홀릭 QR의 경계 좌표만 이용해 중앙·거리·이동 방향을 안내하며, 중앙 판독 범위에 든 QR만 인증으로 전달한다. 관리자 비동기 단일 실행, 목록 조회 실패 복구, 반 소속 안전성, Web 결과 저장과 복구, QR 재발급 확인, 다중 반·보강 학생 원자성, secure session handle, QR PDF 공유와 65×90mm·30×30mm 좌표 보정을 포함한다.
- `webpoc`: 화면 꺼짐을 막고 Lock Task allowlist 안에서 실행된다. 현재 소스와 A 설치본은 `0.4.0-rc31`이다. 학생 화면은 학습지·진단평가 전환만 남기고 Matholic 상단 메뉴, 문제지·오류신고·동영상·답안필기·풀이 업로드를 숨긴다. 주관식 입력은 수식 입력기로 전환하고 제출 화면을 자동 이동한다. 결과 페이지는 풀이·정답을 먼저 차폐한 뒤 지연 생성되는 모든 문제 카드를 순차 로딩해 번호가 1번부터 끝까지 완전할 때만 오답 목록을 표시하며, 불완전하면 `RESULT_INCOMPLETE`로 거짓 결과를 막는다. 브라우저 주소 머리말이 붙는 JavaScript 확인창은 허용된 학생 문서에서만 앱 확인창으로 대체한다. 기존 origin·경로·호출자·프록시·세션 실패폐쇄 경계는 유지한다.
- 외부 알림은 현재 요구사항에서 제외했다. release RC02는 정의된 운영 인수시험을 완료했고 Kiosk RC29/Web POC RC31 조합은 자동검증·보존형 설치까지 완료했다.
