...
- `kiosk`: Gate 4 기능과 Gate 5 Device Owner·전용 HOME·Lock Task를 제공한다. 현재 소스는 `0.6.0-rc78`이다. 학생 CSV 가져오기, 신규·변경 QR 카드 PDF 생성, 카드 상태·이력, 관리자 30초 실행취소, 운영 자가진단·원버튼 복구, PC 상태·암호화 전송과 수식 키패드 배치 설정을 제공한다. QR 대기 화면의 관리자 PIN 메뉴에서 다음 고정 반으로 바로 전환하거나 현재 수업에만 유효한 보충 학생을 추가할 수 있다. 반 변경·보충 추가 성공 안내는 3초 뒤 자동으로 사라진다. 학생 QR 검증이 끝나면 지정 PC에 이름을 제외한 로그인 알림을 한 번만 요청한다. 실제 결과 확인 뒤 채점을 끝내면 PC 상태·알림에 오답 개수와 문제번호 요약을 표시한다. Android 직접 인쇄는 제거됐고 기존 반·학생·QR 데이터와 secure session 경계는 유지한다.
...
- release RC02는 정의된 운영 인수시험을 완료했다. 현재 보관 검증 묶음은 Kiosk RC78/Web POC RC137/PC 수신기 0.1.6이고, 실제 A 설치본은 Kiosk RC77/Web POC RC136이다. RC78/RC137은 자동·릴리스 검증을 통과했지만 A가 연결되지 않아 아직 설치·실물 확인하지 않았다. 기존 A에서 도움말 재터치, PIN 메뉴, 11개 대상 반 목록, 반 전환, 임시 보충 학생 추가·세션 종료 시 제거, 시험계정 Web 정상 로그아웃과 보충 목록 로딩 문구 정리를 통과했다. RC61 QR 카드의 실제 출력·절단·카메라 인식까지 통과했다.
