...
- `kiosk`: Gate 4 기능과 Gate 5 Device Owner·전용 HOME·Lock Task를 제공한다. 현재 소스는 `0.6.0-rc43`이다. 월1~토2 고정 12개 반 빠른선택, 별도 테스트반 생성, 수업 시작 사전점검, PIN 기반 현재 수업 학생 수동 선택, 선택 반 QR 전체 재발급·A4 9장 배치 인쇄를 제공한다. 관리자 비동기 단일 실행, 목록 조회 실패 복구, 반 소속 안전성, Web 결과 저장과 복구, 다중 반·보강 학생 원자성, secure session handle과 QR PDF 공유를 유지한다.
- `webpoc`: 화면 꺼짐을 막고 Lock Task allowlist 안에서 실행된다. 현재 소스는 `0.4.0-rc64`이다. 문제 번호 배지와 112×96px 이전·다음 버튼, 접이식 답안 현황 지도, `모름` 안내와 직접 터치할 때만 열리는 하단 주관식 수식 키패드를 표시한다. `모름`은 답변 완료로 집계하며 현재 문제와 전체답안에 나타난 저장 답안을 다시 판독한다. 네트워크·TLS·공식 서비스 연결 실패는 답안 보호 전용 화면과 비식별 상태 코드로 처리한다. 주관식 MathQuill 복구, 최소 220×56px 입력 영역, 제출 연타 차단, 로그아웃·관리자 복구의 렌더러 재생성 및 기존 origin·경로·호출자·프록시·세션 실패폐쇄 경계를 유지한다.
