...
같은 signer의 Kiosk RC98·Web POC RC139가 보존형 설치돼 있다. Kiosk DB, Device
Owner와 전용 HOME을 보존했고 두 설치본은 서명·버전·보관 artifact 검증을
통과했다. 현재 보관 검증 묶음은 Kiosk RC98, Web POC RC139와 PC 수신기
0.1.8이다.
...
현재 소스 작업본의 Kiosk RC98은 반별 시작 시각만 설정하는 3시간 자동 반 시간표,
...
돌아간다. 또한
자동 전환 중에도 관리자 화면에서 반을 자유롭게 선택해 학생 구성을 관리할 수 있다.
오늘 자동 전환 끄기 확인은 선택한 반의 수업을 실제 시작하거나 진행 중 수업의 반을
직접 바꾸는 동작에만 적용하고, 진행 중 수업의 안전 종료는 허용한다. 또한
...
추가하고 관리자 작업을 닫으면 남은 수업 시간 안에 자동 시작을 다시 확인한다.
빈 반 건너뜀 PC 알림은 같은 예약 수업 구간에 최초 1회만 표시하고, 반복 상태
갱신은 Windows 알림 없이 전송한다. 다음 예약 수업 구간에서는 다시 1회 알린다. 또한
...
- 현재 A: Kiosk `0.6.0-rc98`/code 103, Web POC `0.4.0-rc139`/code 156
- 내부 보관 현재 검증 묶음:
  Kiosk `0.6.0-rc98`/code 103, Web POC `0.4.0-rc139`/code 156,
  PC 수신기 `0.1.8`
- 현재 운영 PC 설치본: PC 수신기 `0.1.8`
- signer SHA-256: `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`
- Kiosk RC98 APK: 36,901,893 bytes,
  `A8E9A360D8BC5173362039E0D110375E0A8EE465C51B9939E74A0A9D06C3C61C`
- Web POC RC139 APK: 3,396,754 bytes,
  `2967F5F2011E870B0C6429797055082118955B3D990CF411F751DC791497132E`
- 현재 A: release signer의 Kiosk RC98/Web POC RC139, 기존 Device Owner·전용
