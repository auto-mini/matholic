...
- 최종 0.3.3 실제 시험계정 A↔B 시도 5: **100/100, A 50회·B 50회, 정상 회귀 PASS**
...
시도 3의 성공으로 정상 교차 기준은 충족했다. 다만 성공 1회만으로 앞선 timeout의 단일 원인을 확정하지 않는다. 이후 0.3.2와 최종 0.3.3에서도 각각 정상 100/100을 재현했으며 G302~G308을 모두 통과해 정의된 Web Gate 3 범위는 **PASS**다.
...
- 실행: 2026-07-23, 최종 0.3.2 / code 14
- A 슬롯: A 자격정보 + B 기대 표시명
- B 슬롯: B 자격정보 + A 기대 표시명
- 종료: A 로그인 1회 뒤 `FAILED / LOCKED / STUDENT_MISMATCH`
- 완료: 0/100, B 로그인 0
- 3초 뒤 terminal 상태 고정: PASS
- 저장 키: 상태·일반화 사유·Gate 3 status/completed/target/duration 6개, 자격정보 키 0
- 앱 데이터 초기화와 공개 preflight 뒤 `IDLE` 복구: PASS

따라서 오계정 허용 없이 첫 표시명 불일치에서 즉시 실패 폐쇄하고 다음 계정을 차단하는 G302는 **PASS**다.
...
따라서 G303과 실계정 session 기반 G304·G305는 PASS다.
...
최종 0.3.3 정상 100/100과 G301~G308의 정의된 실계정·기기 실패주입을 모두 통과했다. 따라서 정의된 Web Gate 3 범위는 PASS다. 실제 학생 DB, QR, Device Owner와 생산 적용은 별도 단계이며 이 판정에 포함하지 않는다.
