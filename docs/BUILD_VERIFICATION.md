...
- 운영 주간 시간표 `월1~토1 10:00`, `월2~토2 13:00`, 각 3시간은 유지하고
  오늘 임시 시간표만 사용해 종료 경계를 짧게 만들었다. 첫 시험은 `목2 12:35`
  시작으로 설정하고 QR 대기에서 15:35 경계를 관찰했다.
- 15:35:19에 수업이 자동 종료됐고 Kiosk가 `ADMIN_IDLE` 관리자 PIN 화면으로
  이동했다. 화면에는 `예약된 수업 시간이 끝나 현재 수업을 자동으로 종료했습니다.`가
  표시됐다. 즉 QR 대기 상태의 종료·Web 안전정리·관리자 인증 이동을 실제 A에서
  확인했다.
- 두 번째 시험은 오늘만 `목2 12:45` 시작으로 설정했다. 표시명이 정확히 `테스트`인
  승인 시험 계정을 현재 수업 보충 명단에만 임시 추가하고, 기존 테스트 QR PDF의
  hash-only 원격 경로로 실제 25문항 문제 화면에 진입했다. 답안은 입력하지 않았다.
- 종료 경계가 지난 15:45:27에도 top resumed는 Web POC였고, 문제 1/25·답안 현황
  0/25 화면이 유지됐다. 따라서 학생 채점 중 자동 종료가 실행되지 않고 보류됨을
  실제 시간 경계에서 확인했다.
- 15:46에 `채점 끝내기`의 강한 경고를 확인한 뒤 `채점 종료·로그아웃`을 실행했다.
  15:46:35에는 Kiosk가 top resumed가 됐고 수업을 자동 종료한 뒤 같은 관리자 PIN
  화면과 자동 종료 안내가 표시됐다. 즉 학생 종료 완료 뒤 보류 전환이 즉시 수렴했다.

### 원상복구·최종 상태

- 두 번의 오늘 임시 시간표를 각각 삭제해 원래 주간 시간표로 복원했다. 자동 종료로
  시험 계정의 이번 수업 보충 명단도 함께 폐기됐다. 현재 시각의 예약 반 `목2`가
  다시 자동 시작됐고 관리자 메뉴 제목 `목2 · 관리자 작업`과 전면 카메라 QR 대기를
  확인했다.
- 최종 A는 Kiosk RC91/code 96 top resumed, Lock Task `LOCKED`, 원격 지원
  `INACTIVE`다. 로컬 원격 캡처, ADB forward/reverse와 Kiosk test package는 없다.
  QR 원문·hash·자격정보는 출력하거나 파일에 저장하지 않았고 기존 학생·반·QR은
  변경하지 않았다.
...
- QR 대기에서 반 변경이나 보충 학생 추가가 성공하면 결과 안내를 한 번 표시하고
  3초 뒤 자동으로 지운다. 안내 세대와 현재 문구를 함께 확인하므로 앞선 타이머가
  이후 새 안내를 지우지 못한다. 실패·후속 조치가 필요한 문구는 유지한다.
- `채점 끝내기` 첫 터치 확인창 제목을
  `주의: 답안을 입력하는 버튼이 아닙니다`로 강화했다. 답안은 문제 화면의
  `입력` 버튼으로 입력한다고 안내하고, 취소는 `문제로 돌아가기`, 실제 종료는
  진한 빨간색 `채점 종료·로그아웃`으로 구분한다. 첫 터치만으로 종료되는 동작은
  계속 차단한다.

### 자동·릴리스 검증

- Kiosk debug 단위시험과 Android 계측시험 Kotlin 소스 컴파일을 통과했다.
  Web debug 단위시험과 변경한 종료 확인 계측시험 Kotlin 소스 컴파일도
  통과했다. 계측시험은 연결된 시험 기기가 없어 실행하지 않았다.
- `scripts/build-release.ps1`의 158개 Gradle 작업 중 156개를 실행하고 2개는
  최신 상태였다. 두 앱 단위시험, release lint, signed assemble,
  non-debuggable·동일 signer 검증을 통과했다.
- Kiosk `0.6.0-rc78`/code 83, 36,704,813 bytes:
...
- 2026-08-09 빌드 시점에 `adb devices -l`에 연결 기기가 없어 RC78/RC137을
  A에 설치하거나 3초 소멸·종료 확인창을 실물 확인하지 않았다. 마지막 독립
  확인 기준 A는 Kiosk RC77/Web RC136이다.
- `8f1c091` QR 성공 안내 3초 자동 소멸
- `9f924df` 채점 종료·로그아웃 경고 강화
- `f1d3f6e` Kiosk RC78/Web RC137 release 복구점

## Kiosk RC77 / Web RC136 A 실기와 선택 목록 교정 — 2026-08-06

### 현장 발견·교정

- RC75를 A에 보존 설치한 뒤 실제 PIN 메뉴를 열었다. 메뉴 네 항목은
  정상 표시됐지만 반 선택과 보충 학생 다중선택 대화상자에 Android
  `AlertDialog`의 설명문과 목록을 동시에 지정해 설명문만 렌더링되고 선택
  목록이 가려지는 문제를 발견했다.
- 두 대화상자에서 별도 message를 제거하고 의미를 제목으로 옮겨 목록을 실제
  content로 렌더링했다. 이 교정을 Kiosk RC76/code 81로 분리 커밋·빌드했다.
- RC76 실기 중 보충 학생 후보 조회가 끝난 뒤 대화상자를 취소하면
  `추가할 보충 학생을 확인하고 있습니다`가 QR 화면에 남는 문제도 발견했다.
  후보 목록을 표시하기 직전에 로딩 문구를 비우고 Kiosk RC77/code 82로
  분리 커밋·빌드했다.

### 자동·릴리스 검증

- Kiosk 94개 debug 단위시험, Android 계측시험 Kotlin 소스 컴파일과 release
  lint를 통과했다. Web RC136의 67개 단위시험과 release lint도 재통과했다.
- `scripts/build-release.ps1`의 158개 Gradle 작업 중 154개를 실행하고 4개는
  최신 상태였으며, signed assemble, non-debuggable·동일 signer 검증을
  통과했다. Web RC136 payload는 직전 보관본과 동일해 기존 artifact를 보존했다.
- Kiosk `0.6.0-rc77`/code 82, 36,704,813 bytes:
  `F053929FD768C6D3F082DEF5D0C3F8A5110093ACD944AB26FA81F8EA27E551AE`.
- Web `0.4.0-rc136`/code 153, 3,393,442 bytes:
  `616914ABEE78052B7DE27C5600084080E477A69660DD33E51E12E72BFCE30491`.
- release signer SHA-256:
  `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`.

### A 보존 설치·실기

- A의 USB 연결 서비스가 처음에는 장치 설명자 단계에서 멈췄지만 A 재부팅 뒤
  정상 연결됐다. RC72/RC134에서 RC75/RC136, 이어 RC76·RC77로 `adb install -r`
  보존 설치했다. Kiosk UID 10288, Web UID 10293, 각 firstInstallTime, Kiosk DB,
  Device Owner와 전용 HOME을 유지했다.
- 재부팅으로 `RECOVERY_REQUIRED`가 된 기존 목2 수업은 관리자 PIN과 원버튼
  안전 복구로 Web 상태·수업을 정리했다. 학생·반·QR 데이터는 삭제하지 않았다.
- QR 도움말은 같은 `?` 버튼으로 열기→닫기, 동일 위치·활성 상태와 접근성
  설명 `도움말`↔`도움말 닫기` 전환을 실제 A에서 확인했다.
- RC76 보충 학생 목록은 목2에서 13명이 실제 다중선택 항목으로 표시됐다.
  시험용 학생 한 명을 현재 수업에 추가했고 성공 안내를 확인했다.
- 반 전환 목록은 현재 목2를 제외한 11개 고정 반을 표시했다. 목2→금1 전환,
  새 QR 대기, 금1→목2 복귀를 통과했다. 전환 뒤 목2 보충 후보가 원래 13명으로
  복원되어 임시 학생이 다음 세션이나 영구 소속에 남지 않았음을 확인했다.
- 저장된 `테스트` 계정만 현재 수업에 임시 추가해 Web 학습지 화면에 진입했다.
  Web 도움말은 첫 `?` 터치 뒤 지속 애니메이션으로 UIAutomator idle 대기가
  발생했고, 같은 위치 재터치 뒤 접근성 덤프가 즉시 정상화돼 동일 버튼 닫기를
  확인했다. 답안은 입력하지 않고 확인 대화상자의 `채점 끝내기`로 정상
  로그아웃했다.
- 시험용 임시 학생은 금1→목2 전환으로 정리했다. 최종 상태는 A Kiosk RC76,
  Web RC136이었다.
- RC77 설치 전 수업을 안전 종료하고 보존 설치했다. 목2 수업을 공식 Web
  사전검사 경로로 다시 시작해 보충 후보 13개가 실제 다중선택 항목으로
  표시됨을 확인했다. 대화상자가 열린 동안과 취소한 뒤의 scanner message는
  모두 빈 값이었다. 후보는 선택하지 않아 학생·반 소속을 변경하지 않았다.
- 최종 상태는 A Kiosk RC77/code 82, Web RC136/code 153, 목2 QR 대기,
  Device Owner 유지, Lock Task `LOCKED`다.

### 운영 PC 종단간 사용자 확인

- 사용자는 RC77/RC136 실제 운영 흐름에서 학생 QR 로그인 Windows 알림이
  정상적으로 한 번 표시되고, 실제 채점 완료 뒤 상세 결과 알림도 정상 표시됨을
  확인했다. 이는 사용자 현장 확인이며 Codex가 Windows 알림 UI를 독립 캡처한
  결과로 확대하지 않는다.
- 사용자는 5분 무입력 경고와 1분 유예 뒤 자동 로그아웃에서는 별도 Windows
  알림이 표시되지 않음을 확인했다. 이는 `pendingGradingCompletion`을 비운 뒤
  공식 로그아웃하고, Kiosk가 결과 메시지가 있을 때만 `notify=true`를 보내는
  현재 소스 및 제품 결정과 일치한다. 자동 종료 뒤 PC 상태는 무알림 QR 대기로
  갱신하며, 자동 로그아웃 자체를 별도 운영 알림으로 만들지는 않는다.

### 복구점

- `25d7d73` 실제 A에서 발견한 반·보충 선택 목록 렌더링 교정
- `4e1d9b1` Kiosk RC76 release 복구점
- `439117b` 완료된 보충 후보 조회 문구 정리
- `506de8b` Kiosk RC77 release 복구점

## Kiosk RC75 / Web RC136 도움말 토글·QR 대기 빠른 수업 작업 — 2026-08-06

### 교정 범위

- QR 대기와 학생 Web 화면에서 도움말을 연 `?` 버튼을 다시 누르면 같은
  도움말이 닫힌다. 도움말 표시 중에도 해당 버튼을 전면·활성·접근 가능 상태로
  유지하고, 접근성 설명은 `도움말 닫기`로 바뀐다. 기존 카드 안의 `닫기`
  버튼도 그대로 유지한다.
- QR 대기 화면의 기존 관리자 아이콘과 PIN 인증 뒤 메뉴에 `다음 수업 반으로
  바로 변경`, `현재 반에 보충 인원 추가 (이번 수업만)`을 추가했다. 별도
  학생용 제어나 PIN 우회 경로는 만들지 않았다.
- 반 전환은 현재 세션이 `QR_READY`이고 학생 처리 중이 아닐 때만 허용한다.
  새 session ID로 다음 수업을 시작하고 이전 수업의 임시 보충 명단을 같은 DB
  transaction에서 정리한다. 대상 반에 활성 소속 학생이 없거나 세션 ID가
  바뀌면 전환 전체를 거부해 기존 수업을 보존한다.
- 보충 인원 후보는 활성 학생 중 현재 반 소속과 이미 추가된 보충 학생을
  제외한다. 선택 결과는 현재 session ID에만 추가하며 영구 반 소속은 바꾸지
  않는다.

### 자동·릴리스 검증

- Kiosk 94개와 Web POC 67개 debug 단위시험이 모두 통과했고 실패·오류·건너뜀은
  0개다. 두 모듈 Android 계측시험 Kotlin 소스 컴파일도 통과했다.
- 저장소 계측시험에 새 세션 ID 발급, 이전 보충 명단 제거, 다음 반 학생 QR
  허용, 빈 반·stale session 거부와 실패 시 원래 세션 보존을 추가했다. 실제
  기기 계측시험은 연결 기기가 없어 실행하지 못했다.
- `scripts/build-release.ps1`의 Kiosk/Web 단위시험, release lint, signed
  assemble, non-debuggable·동일 signer 검증을 통과했다. 최종 완주에서 158개
  Gradle 작업 중 152개를 실행했고 6개는 최신 상태였다.
- Kiosk `0.6.0-rc75`/code 80, 36,704,813 bytes:
  `8474A83C0E52BFB50F06B93DBA8E578F9F4CA5B642DF3E9A3D6A9C6D9CAB9B07`.
- Web `0.4.0-rc136`/code 153, 3,393,442 bytes:
  `616914ABEE78052B7DE27C5600084080E477A69660DD33E51E12E72BFCE30491`.
- release signer SHA-256:
  `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`.

최초 정식 빌드는 158개 Gradle 작업을 모두 통과했지만 4분 명령 제한 때문에
후속 APK 검증 직전에 종료됐다. 시간 제한을 늘린 동일 정식 절차를 다시 실행해
Gradle·APK 검증·산출물 보관까지 완주했다.

### 실기 제한과 복구점

- A `R54TB029FHZ`는 ADB 기기 목록에 없었다. 따라서 RC75/RC136 보존 설치,
  `?` 버튼 재터치, 12개 고정 반 선택 UI, 실제 PIN 반 전환과 보충 학생 QR
  로그인의 물리 실기는 수행하지 못했다. 현재 A 설치본은 계속 Kiosk RC72/Web
  RC134다.
- `9b4f90f` 도움말 버튼 토글 교정
- `47abdc3` PIN 기반 빠른 반 전환·임시 보충 인원 기능과 저장소 회귀시험
- `a7d3a9f` Kiosk RC75/Web RC136 release 복구점

## Kiosk RC74 / Web RC135 채점 결과 PC 알림 — 2026-08-06

### 교정 범위

- Web 결과 화면에서 `확인하고 채점 끝내기`를 누른 경우에만 정제된 결과를
  Kiosk에 돌려준다. 수동 `채점 끝내기`와 5분 무입력 뒤 자동 종료에는 결과를
  붙이지 않아 기존의 무조건적인 `채점 완료` 오인 알림을 제거했다.
- 결과에는 오답 개수와 문제번호만 포함한다. Kiosk는 PC 알림의 80자 제한에
  맞춰 문제번호를 최대 8개 표시하고 나머지는 `외 N개`로 요약한다. 틀린 문제가
  없으면 `틀린 문제 없음`, 결과를 완전히 판독하지 못하면 임의 수치 대신
  `상세 결과 확인 필요`로 실패폐쇄한다.
- 결과는 명시적 Activity result로 Web POC에서 Kiosk에 전달된 뒤 기존 PC
  페어링 암호화·인증 채널을 사용한다. Windows 알림에는 학생 이름을 넣지 않고,
  PC 상태창에서만 학생 이름과 결과 상태를 함께 표시한다.

### 자동·릴리스 검증

- Kiosk 94개와 Web POC 67개 debug 단위시험이 모두 통과했고 실패·오류·건너뜀은
  0개다. 두 모듈 debug lint와 Android 계측시험 Kotlin 소스 컴파일도 통과했다.
- 회귀시험은 오답 번호 정렬·중복 제거·범위 제한, 틀린 문제 없음, 200개 초과
  실패폐쇄, 최대 8개 표시와 80자 제한, 결과 없는 수동·무입력 종료의 알림 배제를
  확인한다.
- `scripts/build-release.ps1`의 Kiosk/Web 단위시험, release lint, signed
  assemble, non-debuggable·동일 signer 검증을 통과했다. 158개 Gradle 작업 중
  155개를 실행했고 3개는 최신 상태였다.
- Kiosk `0.6.0-rc74`/code 79, 36,688,429 bytes:
  `017D4D49401B454348E3A11CE7E35419EB9FE5BA55E68704E6420A99D3940BA3`.
- Web `0.4.0-rc135`/code 152, 3,393,154 bytes:
  `3E9A2C60C46D624B1103AFCA7C15C3DBD14177621A1EA1E0A12DD10B1E2C8F31`.
- release signer SHA-256:
  `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`.

### PC 확인과 실기 제한

- 운영 PC 수신기에 실제 학생 데이터 없이
  `채점 완료 · 오답 3개: 2, 5, 9번/notify=true` 인증 이벤트를 정확히 1건
  전송했고 수신기가 수락했다. 이어서 무알림 `QR 대기` 상태를 전송해 PC 상태를
  복원했다. PDF나 학생 파일은 생성하지 않았다.
- A `R54TB029FHZ`는 이번 검증 시점에도 ADB 기기 목록에 없었다. 따라서
  RC74/RC135 보존 설치와 실제 매쓰홀릭 결과 화면→PC 알림 종단간 실기는 수행하지
  못했다. 현재 A 설치본은 계속 Kiosk RC72/Web RC134다.

### 커밋·복구점

- `8d90727` 실제 채점 결과만 PC로 전달·요약·알림하는 기능과 회귀시험
- `bde7a4c` Kiosk RC74/Web RC135 release 복구점
...
- 전면 카메라 렌즈 화살표를 직전 보정 위치에서 2dp 위로 올렸다. A 기기
  240dpi에서 약 0.5mm에 해당한다.
- 문제 좌상단 읽기 전용 `N번` 배지를 뒤로 버튼 오른쪽으로 옮기고 두 요소가
  겹치지 않게 했다.
- 객관식 도움말은 문제 본문 안의 보기뿐 아니라 오른쪽 번호 선택지도 함께
  강조하고, 어느 쪽을 눌러도 같은 답이 선택됨을 설명한다.
- 모든 클릭 가능 요소는 pointerdown 즉시 축소·명도 변화·노란 링으로 반응한다.
  공식 이전·다음 버튼은 목표 문제번호를 즉시 배지에 반영하고 전환 중 중복
  입력을 막는다.
- MathQuill 답안 끝의 입력 경로상 만들 수 없는 연속 빼기 기호(`--` 이상)를
  발견하면 연속 기호를 제거한다. 정상 음수 하나(`-6`)는 보존한다.
- 학생 화면의 무입력 경고를 10분에서 5분으로 낮췄다. 경고 뒤 1분 안에
  `계속 채점`을 누르지 않으면 공식 로그아웃을 수행하고 Kiosk QR 화면으로
  돌아간다. 안내 제목·본문도 5분/1분 정책에 맞췄다.

### 자동·릴리스 검증

- Kiosk/Web debug 단위시험, 관련 Android 계측시험 소스 컴파일과 lint를
  통과했다. 무입력 계약 시험은 5분 경고 문구, `계속 채점`에 의한 타이머
  재설정, 1분 유예 만료 뒤 `LOGOUT_NAVIGATE` 전이를 확인한다.
- `scripts/build-release.ps1`의 단위시험, release lint, signed assemble,
  non-debuggable·동일 signer 검증을 통과했다.
- Kiosk `0.6.0-rc72`/code 77, 36,672,045 bytes:
  `E95CD861E530535198E29E037E225D5A4980EBA742CDF29BF2866E2807E9FCB0`.
- Web `0.4.0-rc134`/code 151, 3,388,642 bytes:
  `5460D9EBF6A0A791C274AE4629A22D07E3BDC7FA7C8ACC14028035954CFF6E5C`.
- release signer SHA-256:
  `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`.
...
- Samsung SM-P610 `R54TB029FHZ`에 `adb install -r`로 두 release APK를
  설치했다. Kiosk UID 10288·first install `2026-07-24 12:52:28`, Web UID
  10293·first install `2026-07-28 13:12:16`, Kiosk DB와 Device Owner를
  보존했다.
- 정확한 표시명이 `테스트`인 계정만 허용하는 승인 원격 QR 회귀 경로로 실제
  학습지에 들어갔다. QR 원문·자격정보는 출력하거나 저장하지 않았다.
- 현재 물리 방향에서 전면 렌즈 화살표 위치를 확인했다. 문제번호 배지와 뒤로
  버튼은 DOM 실측 16px 간격으로 겹치지 않았다. pointerdown 즉시 이전·다음
  버튼에 축소·명도·노란 링이 보였고, 다음 문제 번호와 내용이 전환됐다.
- 실제 이지선다 문제의 도움말에서 문제 안 보기와 오른쪽 번호 선택지가 모두
  강조됨을 확인했다. 실제 MathQuill 필드에 회귀값 `6--`를 주입하면 `6`으로
  1회 복구됨을 확인했다. 필드 API로 빈 값 복원을 요청했지만 공식 React 초안
  상태가 복구된 `6`을 다시 표시했다. 이 시험 답안은 제출하지 않았고, 현재
  세션 폐기 사용자 승인에 따라 Web 로그인과 현재 수업을 안전 종료했다.
- 마지막 빈 화면 터치를 0초로 삼아 실제 시간을 단축 없이 측정했다. 290초에는
  경고가 없었고, 306초에는 `5분 동안 입력이 없었습니다`와 1분 내 계속 채점
  안내가 표시됐다. 버튼을 누르지 않자 373초에는 Web이 종료되고 Kiosk
  `QR_READY`로 복귀했다.
- 검증 수업은 `현재 수업 안전 종료`로 정리했다. 최종 상태는 Kiosk RC72,
  Web RC134, Kiosk `ADMIN_IDLE`, Device Owner 유지, Lock Task `NONE`, 원격 지원
  `INACTIVE`, ADB TCP 9222 전달 없음이다. 이번 검증에서 생성한 QR 렌더·화면
  캡처는 삭제했고 기존 원격 점검 자료와 사용자 `outputs/`는 건드리지 않았다.
- 렌즈 보정은 현재 물리 방향에서 확인했다. 반대 방향은 이번 회차에 기기를
  직접 다시 돌려 확인하지 않았으며, 동일한 2dp 보정 코드가 양 방향 계산에
  공통 적용되는 것까지만 소스·자동시험으로 확인했다.

### 커밋·복구점

- `1ffa523` 전면 렌즈 화살표 0.5mm 미세 보정
- `3692e74` 문제번호 배지와 뒤로 버튼 분리
- `a787256` 객관식 양쪽 선택지 도움말
- `bf5c132` 즉시 터치·문제 전환 피드백
- `866484f` 불가능한 연속 답안 부호 복구
- `925bff5` Kiosk RC72/Web RC133 release 복구점
- `45b8e35` 5분 경고·1분 뒤 안전 로그아웃과 QR 복귀
- `285a225` Web RC134 release 복구점

## 운영 PC·실제 A 추가 장애·부하 현장검증 — 2026-08-05

### PC 수신기 부하·연결 한계

- 정식 설치된 PC 수신기 0.1.6에 5MiB PDF 32개를 동시에 전송했다. 32/32가
  인증 ACK를 반환했고 수신 파일 32개의 전체 내용 hash가 원본과 일치했다.
  총 소요 1.125초, 단일 요청 최대 0.890초였고 process RSS는 59.3MiB에서
  59.6MiB로 유지됐다. 시험 파일은 정확한 표식과 크기를 확인한 뒤 모두
  정리했고 잔존은 0개다.
- partial connection 32개를 동시에 유지한 상태에서 33번째 연결이 즉시
  닫히는 것을 확인했다. 32개를 닫은 뒤 설치본 `--smoke-check`는 exit 0,
  listener는 계속 정확히 1개였다.
- 첫 부하 harness는 수신기가 안전하게 정리한 파일명과 harness의 선행 밑줄
  기대가 달라 파일 검증을 0개로 오판하고 exit 1이었다. 실제 생성 파일 32개와
  크기는 별도 확인됐고, 파일명 기대를 현재 수신 규칙에 맞춘 재실행에서 위
  32/32 내용 검증이 통과했다. 제품 실패로 보지 않는다.

### 실제 A의 QR 로그인·네트워크 중단 입력 차단

- 승인 A `R54TB029FHZ` / Samsung `SM-P610`에서 일회성 시험 반
  `SOL-TEST-0805-0023`에 표시명이 정확히 `테스트`인 계정 한 명만 연결했다.
  해당 계정 QR을 새로 발급하고 지정 PC로 암호화 전송한 뒤, 공식
  `submit-test-qr-from-pdf.py` 경로로 정확한 카드명과 QR 1개 조건을 확인해
  로그인했다. QR 원문·hash는 출력하거나 저장하지 않았다.
- 미제출 25문항 과제의 빈 2번 주관식에서 답안 현황 `0/25`와 빈 입력을 먼저
  확인했다. A의 기본 입력기는 Samsung IME였고 입력기가 활성화된 상태에서
  Matholic 자체 수식 키패드가 표시됐다.
- A Wi-Fi를 실제로 끄자 불투명한 `인터넷 연결 복구를 기다리는 중입니다 ·
  답안은 그대로 유지됩니다` 화면으로 전환됐다. 이 상태에서 ADB text 입력,
  key event, 가려진 화면 키패드 좌표 tap, Enter와 Delete를 주입했다.
- Wi-Fi 재활성화 뒤 앱 화면은 3초 안에 같은 2번 문항으로 복구됐고 입력값은
  빈 상태, 답안 현황은 계속 `0/25`였다. 시험 중 답안을 한 글자도 저장하거나
  제출하지 않았으므로 별도 답안 원상복구가 필요하지 않았다.
- 정상 `채점 끝내기`와 Kiosk 수업 안전 종료를 거쳐 시험 반을 삭제했다. 전체
  반 목록에서 해당 반이 없음을 확인했고, A는 Kiosk 관리자 인증 잠금 화면,
  Lock Task `LOCKED`, 원격 지원 `INACTIVE`로 복원했다. 새 `테스트` QR PDF는
  현재 유효한 시험카드로 PC의 지정 수신 폴더에 남겼다.

### 화면 조건·PC 네트워크 전이

- Android 13 `matholic_rc03_api33`에서 Kiosk
  `MainActivityInstrumentedTest` 16개를 세 조건으로 각각 실행했다.
  기본 1080×1920/420dpi, A 대응 2000×1200/240dpi + font scale 1.3,
  같은 해상도 + font scale 0.85 + 180도 반전 가로 방향에서 총 **48/48 PASS**다.
  AVD 설정은 size/density/font/rotation 기본값으로 복원한 뒤 종료했다.
- PC Wi-Fi를 실제 disconnect한 뒤 같은 저장 profile로 reconnect했다. 단절 중
  설치 수신기 process와 `0.0.0.0:48129` listener가 유지됐고 설치본
  `--smoke-check`는 exit 0이었다. Internet 재연결은 1초 안에 확인됐고 이후
  listener와 smoke도 다시 통과했다.
- DHCP가 활성인 실제 Wi-Fi에서 `ipconfig /release`로 비-link-local IPv4가
  0개가 된 것을 확인한 뒤 `/renew`했다. 두 명령 모두 exit 0, Internet과 IPv4는
  1초 안에 복구됐고 수신기 listener와 smoke도 유지됐다.
- 활성 IPv4 경로는 실제 Wi-Fi 1개와 가상 interface 1개였고, 수신기 TCP 48129는
  두 주소 모두에서 연결됐다. 실제 활성 VPN adapter는 0개라 VPN 경유 선택은
  수행하지 않았다.

### 남은 물리 조건

- 새로 출력한 종이 QR의 인쇄·반사·거리 실기는 별도 물리 조건이다.
...
- 답안 현황판이 열렸을 때 도움말은 현황판 전체만 한 번 강조하고 뒤쪽 문제·답안
  컨트롤의 도움말은 표시하지 않는다. 현황판 밖을 누르면 그 한 번의 pointer
  sequence를 끝까지 소비해 뒤 컨트롤로 click이 전달되지 않는 기존 안전 경계를
  유지한다.
- 전체답안 확인에서는 보이는 review modal 안의 제출 후보를 세로 좌표로 비교해
  가장 아래쪽 최종 `답안 제출`만 강조한다. 상단의 1차 제출 버튼을 가리키지 않는다.
- 이지선다 문제도 표준 button뿐 아니라 Ant radio wrapper와 시각 번호 marker를
  의미 기반으로 찾는다. 느슨한 marker 추정은 선택지 수가 정확히 2개일 때만 허용해
  다른 그림의 숫자를 답으로 오인하는 범위를 제한했다.
- 전면 카메라 안내 화살표를 양 회전에 공통으로 13dp 아래로 이동해 보고된 약 2mm
  상향 오차를 보정했다. QR 도움말은 가짜 QR 사진을 제거하고 학생이 코팅 명찰을
  태블릿 전면 렌즈로 가져가는 전용 시뮬레이션 일러스트로 교체했으며, 가로 2열
  레이아웃으로 아래 문구가 잘리지 않게 했다.
- 빈 진단평가는 `학습지`로, 빈 학습지는 `진단평가`로 이동하도록 안내한다.
  `더 누르지 말고 선생님에게 알리기` 문구는 제거했다.
- 주관식 키패드는 답안 현황 아래라는 고정 기준보다 실제 입력창 아래 16 CSS px를
  우선하며, 화면 아래 `채점 끝내기` 영역 88 CSS px를 남긴다. 문제 본문 전체를
  억지로 스크롤하지 않고 입력창만 필요한 만큼 보이게 한다.
- `채점 끝내기`는 `채점을 끝낼까요?` 확인창을 열고 `계속 채점` 또는
  `채점 끝내기`를 선택하게 한다. 취소 직후 내부 dialog 참조도 동기 정리해 빠른
  재조작이 막히지 않는다.
- 답안 채점 spinner·분석 중 화면은 도움말 context를 `NONE`으로 실패 폐쇄한다.
  채점이 끝나 결과 화면이 확정된 뒤에만 결과 도움말을 다시 제공한다.
- 실제 기기 계측에서 추가로 발견한 문제 번호 feedback 지연도 고쳤다. 답안 현황의
  native select 경로가 이동 직전 `N/전체`를 이전 번호로 되돌리던 순서를 수정해,
  선택한 번호를 즉시 유지한다. DOM 계약은 `web-2026-08-04.6`으로 올렸다.

### 자동·릴리스 검증

- Web debug 단위시험 **65 tests, 0 failures, 0 errors, 0 skipped**,
  Kiosk debug 단위시험 **84 tests, 0 failures, 0 errors, 0 skipped**.
- `:webpoc:lintDebug`, `:webpoc:compileDebugAndroidTestKotlin`,
  Kiosk/Web release lint와 assemble: **PASS**.
- 최종 `scripts/build-release.ps1`: **158 tasks PASS**. 버전, non-debuggable,
  동일 release signer 검증까지 통과했다.
- 실제 SM-P610 Web 계측 전수 118개를 두 번 실행했다. 첫 실행은 새로 추가한
  문제 번호 동기화·종료 확인 참조 시험 2개만 실패했고 이를 별도 커밋으로 교정한
  뒤 대상 시험 **2/2 PASS**. 최종 전수 실행은 **116/118 PASS**였으며 남은
  2개는 Lock Task `LOCKED`에서 `ActivityScenario`가 `PRE_ON_CREATE` 또는
  `RESUMED`를 벗어나지 못한 시험 환경 실패였다. 같은 두 시험을 같은 상태에서
  재실행하면 재현됐고, 관리자 인증으로 Lock Task를 `NONE`으로만 바꾼 뒤
  **2/2 PASS**해 기능 실패가 아님을 반대 검증했다.
- Kiosk `0.6.0-rc69`/code 74, 36,671,949 bytes:
  `ACFF14F08101B1C3070839C1EEB49B2339BB9E4619040603712C1A01B87ADA4C`.
- Web `0.4.0-rc132`/code 149, 3,382,506 bytes:
  `3B8CB0822DE1415A68DB664BAC04E3CD41F2527F47F481EB3583722859C4A9E8`.
- release signer SHA-256:
  `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`.

### 실제 태블릿 검증·최종 상태

- Samsung SM-P610 `R54TB029FHZ`에서 정확한 시험용 QR로 Web에 들어가 실제
  Matholic 화면을 검증했다. 답안 현황 도움말은 현황판만 표시했고, 전체답안
  도움말은 중앙 하단 최종 제출 버튼을 가리켰다.
- 실제 이지선다 1번 문제 그림의 `②` 위치를 물리 탭해 선택값 `2`, 두 번째
  overlay zone 활성과 답안 현황 `1/10`을 확인했다.
- 주관식 입력창 화면 좌표 `y=562..658`, 키패드 `y=683..950`, 종료 버튼
  `y=1086..1165`로 입력창과 약 25px, 종료 버튼과 약 136px 간격을 확인했다.
- 종료 확인창의 문구·양 버튼과 취소 후 ACTIVE 유지, 빈 진단평가의 학습지 이동
  안내를 확인했다. 실제 제출로 spinner가 13→3→2개로 줄어드는 구간 동안
  grading context가 `NONE`이었고 결과 확정 뒤 결과 화면으로 전환됐다.
- QR 기본 화면에서 현재 물리 방향의 화살표가 실제 렌즈 높이에 맞고, 새 QR
  도움말의 시뮬레이션 그림과 아래 문구가 잘리지 않음을 확인했다. 이번 RC69에서
  반대 물리 방향은 직접 다시 돌려 보지 못했다. 앱이 `sensorLandscape`라 ADB
  강제 회전이 적용되지 않았으며, 같은 13dp 보정과 기존 좌우 반전 코드가 양
  방향에 공통 적용된 사실까지만 확인했다.
- 검증용 `테스트` 계정으로 학습지 하나를 제출했으며 운영 학생 답안은 건드리지
  않았다. 검증 뒤 Web 로그인, 현재 수업과 일회성 보강 명단을 안전 종료했다.
- `adb install -r`로 Kiosk UID 10288·first install `2026-07-24 12:52:28`,
  Web UID 10293·first install `2026-07-28 13:12:16`과 기존 데이터를 보존했다.
  최종 상태는 Kiosk RC69/code 74, Web RC132/code 149, Kiosk `ADMIN_IDLE`,
  Device Owner 유지, Lock Task `LOCKED`, 관리자 인증 화면, 원격 지원
  `INACTIVE`, ADB TCP 9222 전달 없음이다. 회전 설정은
  `accelerometer_rotation=1`, `user_rotation=0`으로 복원했다.

### 커밋·복구점

- `0db5f63` 상황별 도움말 scope·빈 목록 안내·grading 도움말 차단
- `e833189` 이지선다 그림 번호 선택
- `1a3679e` 주관식 키패드 위치
- `f6e2342`, `809975b` 종료 확인과 취소 참조 정리
- `98d0bef` 렌즈 화살표·QR 도움말 시뮬레이션
- `5692806` 문제 번호 즉시 feedback
- `16de763`, `ea80e14` RC69/RC132 release 복구점

## Kiosk RC68 / Web RC130 학생 조작·도움말 개선 — 2026-08-04

### 범위·교정

- 문제 화면 숫자 키패드는 고정 좌표가 아니라 실제 `답안 현황`
  상자 위치를 읽어 바로 아래에 붙인다. 주관식 입력 칸이 포커스될 때만
  보이고, 도움말·전체답안에서는 숨겨 겹침을 막았다.
- 클릭 가능 요소는 `pointerdown` 즉시 0.94 축소·노란 4px 링으로
  누른 대상을 표시한다. 문제 전환 대기·폴링 간격·재시도를 줄여 실제
  태블릿에서 피드백 14.4ms, 좌상단 문제 번호 변경 280.8ms, 전환
  안정 396.5ms를 측정했다.
- 문제 좌상단에 현재 번호만 크게 `N번`으로 표시한다. 전체 문제
  수를 써 놓지 않고 `pointer-events: none`으로 두어 문제 이동 버튼으로
  오인하거나 터치되지 않는다.
- `빛 반사가 강합니다`는 학생 표시 신호에서 무시하고 QR 도움말
  문구에서도 제거했다.
- 목록 도움말은 상단 탭을 다시 확인하라고 하지 않고, 현재 화면의
  실제 첫 항목·시작 버튼을 박스로 추적한다. 빈 목록은 없는 상태를
  판별해 선생님에게 문의하라는 독립 안내를 보여 준다.
- 문제 도움말은 실제 DOM을 사용한 객관식·주관식으로 나누고, 전체답안
  도움말은 실제 최종 `답안 제출` 버튼만 강조한다. 안내 레이어가
  열린 동안 WebView와 Android 헤더·제출 버튼을 모두 차단한다.
- QR 도움말에는 실제 QR 비트맵과 10–20cm 인식 시뮬레이션을
  넣었다. 기본 QR 대기 화면에는 현재 회전에서 전면 카메라 렌즈가
  있는 쪽을 가리키는 화살표를 표시하고, 후면 카메라에서는 숨긴다.

### 소수·분수 판정 확인 범위

- **확인 사실:** 공식 학생 웹 클라이언트는 주관식 입력을
  `{number, userAnswerValue}`의 `userAnswerValue`로 변환 없이 전송하며,
  실제 정오판정은 Matholic 서버의 채점 API가 한다. 클라이언트에
  `1/2`와 `0.5`를 동치로 만드는 규칙은 없다.
- **미검증:** 서버 내부 답안·채점 규칙은 공개된 클라이언트 코드로
  확인할 수 없어, 특정 문제에서 두 표현이 반드시 같이 정답 처리된다고
  단정하지 않았다.
- **판단:** 키오스크에서 임의로 소수↔분수를 변환하면 `분수로
  쓰시오` 같은 표현 조건을 깨뜨릴 수 있어 자동 보정은 넣지 않았다.
  대신 주관식 도움말이 문제의 표현 조건을 그대로 따라 입력하고,
  입력값은 서버에 그대로 전송된다고 알린다.

### 자동·릴리스 검증

- 관련 debug 단위시험, `:webpoc:compileDebugAndroidTestKotlin`, Kiosk/Web
  debug assemble: **PASS**.
- 최종 `scripts/build-release.ps1`: **158 tasks PASS**. Kiosk/Web 단위시험,
  release lint, signed assemble, 버전·non-debuggable·동일 signer 검증을 포함한다.
- 객관식·주관식 실제 DOM 도움말 계측 회귀시험은 소스 컴파일까지
  통과했다. debug 서명이 설치된 release 앱 데이터를 교체하지 않도록
  실제 태블릿 Android 계측시험 실행은 생략하고 동일 흐름을 수동 실기로
  검증했다.
- Kiosk `0.6.0-rc68`/code 73, 35,230,657 bytes:
  `065FE9976CD34B71C2F273F6725FD70E751B33FF0CDE777547558A87CDDF9581`.
- Web `0.4.0-rc130`/code 147, 3,374,190 bytes:
  `E15ED80899A6F23E6E814927367EA5E7279EE753847B7DBF576ECE39D14F32A2`.
- release signer SHA-256:
  `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`.

### 실제 태블릿 검증·최종 상태

- Samsung SM-P610 `R54TB029FHZ`에 두 APK를 `adb install -r`로 보존형
  설치했다. Kiosk UID 10288·firstInstallTime `2026-07-24 12:52:28`, Web
  UID 10293·firstInstallTime `2026-07-28 13:12:16`을 유지했다.
- 실제 QR 대기에서 현재 방향의 왼쪽 렌즈 화살표를 확인했다.
  기기를 rotation 3으로 잠시 고정했을 때 오른쪽으로 자동 반전됐고,
  후면 카메라에서는 숨었다가 전면으로 복귀했을 때 다시 보였다.
  검증 뒤 회전 설정은 `accelerometer_rotation=1`, `user_rotation=0`,
  `wm user-rotation free`, `wm fixed-to-user-rotation default`로 복원했다.
- 정확한 표시명 `테스트` QR로 학습지에 진입해 좌상단 번호,
  버튼 반응·전환, 키패드 위치, 학습지·빈 진단평가·객관식·주관식·
  전체답안 도움말을 실제 화면에서 확인했다.
- 도움말을 열고 뒤의 진단평가·`채점 끝내기`를 누른 전후 화면
  SHA-256이 같아 Android 컨트롤 차단을 확인했다. QR 도움말 뒤의
  관리자 버튼도 열리지 않았다.
- 답안을 입력·제출하지 않고 검토 창을 닫은 뒤 세션을 종료했다.
  최종 상태는 Kiosk RC68/code 73, Web RC130/code 147, Kiosk
  `QR_READY`, Device Owner 유지, Lock Task `LOCKED`, 최근 AndroidRuntime
  `FATAL EXCEPTION` 0건이다.

## Kiosk RC67 / Web RC125 화면별 학생 도움말 — 2026-08-04

### 범위·교정

- 기존 A4 안내 PDF를 대신해 학생이 실제로 조작하는 여섯 상태에 화면별 도움말을
  제공한다: QR 카드 인식, 학습지 목록, 진단평가 목록, 문제 풀이, 전체답안 확인,
  채점 결과. 로그인 준비·네트워크 복구·무입력 경고처럼 학생 입력을 차단해야 하는
  일시 화면에는 도움말을 노출하지 않는다.
- 좌상단 학생 이름 옆 또는 QR 화면 좌상단의 고정 `?` 버튼이 Android 최상위
  도움말 레이어를 연다. 도움말은 큰 제목·짧은 설명·번호 순서·주의사항으로 구성하며
  화면 뒤쪽의 WebView·카메라·관리자·제출·답 입력에는 터치가 전달되지 않는다.
- `/workbook`, `/diagnostic`, `/learningV2/` 경로와 보이는 `전체답안` 제목을
  의미 기반으로 판별해 같은 버튼의 내용을 전환한다. 결과 판독이 끝나면 Web 화면을
  숨기고 결과 전용 도움말을 표시한다. 알 수 없는 값은 도움말을 숨기는 방향으로
  실패 폐쇄한다.
- QR 도움말이 열리면 QR 분석기를 일시 중지하고 닫을 때 `QR_READY` 상태에서만
  다시 활성화한다. 도움말을 닫는 시스템 Back도 같은 안전 복귀 경로를 사용한다.
- 실제 태블릿에서 텍스트 `?`가 일부 화면에서 `!`처럼 좁게 보이는 현상과 첫 벡터가
  너무 작게 보이는 현상을 재검토해, 최종본은 버튼 위치·터치 영역을 유지한 큰 고정
  벡터 물음표로 교체했다.

### 자동·릴리스 검증

- `scripts/build-release.ps1`: **158 tasks PASS**. Kiosk/Web 단위시험,
  release lint, signed assemble, 버전·non-debuggable·동일 signer 검증을 포함한다.
- Kiosk JVM 단위시험: **84 tests, 0 failures, 0 errors, 0 skipped**.
- Web JVM 단위시험: **62 tests, 0 failures, 0 errors, 0 skipped**. 새 시험은 여섯
  도움말 상태의 문구 존재, 경로·DOM 계약 매핑과 알 수 없는 값의 실패 폐쇄를 검증한다.
- Kiosk `0.6.0-rc67`/code 72, 35,229,245 bytes:
  `6119589171AD36432565BA98E8967A82D5201DADA570AF93EE4B202A87484304`.
- Web `0.4.0-rc125`/code 142, 3,346,162 bytes:
  `CFF2B2AC3ACA6FD560F5422086A8C40B48A3F425AE8A3E64BBFFC9F19385E0DE`.
- release signer SHA-256:
  `9d5bd7d9c328df2e5c54b67d1aa2d42caef2674eeace0614bfe2d37c7651f5b7`.

### 실제 태블릿 검증·최종 상태

- Samsung SM-P610 `R54TB029FHZ`에 두 APK를 `adb install -r`로 보존형 설치했다.
  Kiosk UID 10288·firstInstallTime `2026-07-24 12:52:28`, Web UID 10293·
  firstInstallTime `2026-07-28 13:12:16`, Device Owner와 기존 앱 데이터를 유지했다.
- QR 도움말을 열고 뒤의 관리자 버튼 좌표를 눌러도 관리자 인증이 열리지 않았다.
  Web 학습지 도움말을 열고 뒤의 진단평가 좌표를 눌러도 학습지 선택 상태가 유지됐다.
  문제 도움말을 열고 뒤의 `모름` 좌표를 눌러도 도움말이 유지되고 답안 현황은
  **0/10**으로 남았다.
- 학습지·진단평가·문제·전체답안 도움말을 실제 경로에서 각각 확인했다. 기존에
  제출 완료된 `1/1 (100점)` 항목을 열어 결과 도움말도 확인했으며, 새 답안은
  입력하거나 제출하지 않았다.
- 최종 상태는 Kiosk RC67/code 72, Web RC125/code 142, Kiosk `QR_READY`,
  Device Owner 유지, Lock Task `LOCKED`, 원격 지원 `INACTIVE`다.
