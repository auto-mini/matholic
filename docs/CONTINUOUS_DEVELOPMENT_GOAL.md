...
## 현재 최우선 무인 운영 기준 — 2026-08-01

이 절은 이 문서의 모든 과거 종료·동석·사용자 부재 규칙보다 최신이며,
상충하는 내용이 있으면 이 절을 우선한다. 사용자는 현재 A 기기와 PC를 계속
지켜보거나 직접 조작하지 않으며, 사용자가 명시적으로 중단할 때까지 Codex가
조사 → 실제 화면 조작 → 구현 → 자동시험 → A 설치·실기 → 검토 → 개선을
자율적으로 반복한다.
...
- 원격 실기에서 객관식·주관식 시험값을 만들었으면 화면 표시만 지우고 정리됐다고
  판단하지 않는다. 공식 답안 상태를 `unanswered`로 저장한 뒤 세션을 완전히
  종료하고 같은 과제에 재진입해 최소 60초 동안 해당 답안과 전체 답안 수가
  다시 나타나지 않는 것까지 확인한다. React가 제어를 교체하는 화면에서는
  동일 DOM 참조를 재사용하지 않고 매 단계 최신 공식 제어를 다시 찾는다.
...
- 원격 화면을 직접 보고 관리자, QR 대기, 로그인, 학습지·진단평가, 문제풀이,
  전체답안, 결과, 연속 채점, 안전 종료와 복구 흐름을 실제로 조작한다.
- 단순 기능 성공뿐 아니라 사람의 사용 관점에서 시각 계층, 정렬, 잘림·겹침,
  여백, 터치 영역, 반응 지연, 깜박임, 지연 노출, 키패드, 스크롤, 회전,
  오류 문구와 복구 가능성을 확인한다.
- 각 입력 전 최신 화면과 앱 상태를 다시 확인한다. 접근성·의미 구조 또는
  상태 기반 입력을 우선하고, 그것이 없을 때만 최신 캡처에 결합된 좌표 입력을
  사용한다. 오래된 좌표를 재사용하거나 화면 확인 없는 무인 좌표 시퀀스를
  운영 기능으로 만들지 않는다.
- 실제 학생·반·QR은 보존하고 시험계정·합성 데이터만 변경한다. 시험 과제
  진행·제출 기록은 필요한 만큼 변경할 수 있으나 테스트 목적 없는 제출은
  하지 않는다.
- 테스트 자료가 부족하면 필요한 문항 수, 문제 유형, 정답·오답 조건과 반복
  수량을 미검증 목록에 구체적으로 기록한다. 해당 자료 없이 가능한 다른
  의미 있는 작업은 계속하고 사용자 복귀 때 한 번에 요청한다.

### 시각 품질 합격 기준

- 자동시험의 DOM 크기·존재 assertion만으로 디자인을 통과시키지 않는다.
  각 주요 화면 진입 직후, 제어를 연 직후, 입력기·키패드 표시 직후, 문제 이동
  직후, 제출·결과·복구 전환 직후의 실제 A 캡처를 직접 읽고 판단한다.
- 기준 캡처와 변경 후 캡처를 같은 상태에서 비교한다. 캡처 사이에 애니메이션·
  로딩·지연 DOM이 있는 화면은 즉시와 안정화 뒤를 모두 확인해 잠깐 나타났다
  사라지는 불필요한 요소까지 검사한다.
- 다음 결과는 기능이 동작해도 디자인 실패로 취급한다.
  - 버튼·문제번호·답안 현황·문구·키패드가 다른 핵심 제어 또는 문제 내용을
    일부라도 가리는 상태
  - 좌우·상하 잘림, 불필요한 가로 스크롤, 화면 밖 제어, 애매하게 반만 가리는
    배치와 의미 없는 큰 여백
  - 지나치게 크거나 작은 키패드·버튼, 터치 대상 사이 간격 부족, 실수로
    `채점 끝내기`·제출을 누르기 쉬운 배치
  - 위치·색·텍스트만으로 역할을 이해하기 어렵거나 현재 상태·선택·다음 행동을
    구분하기 어려운 시각 계층
  - 숨겨야 할 사이트 요소가 전환 중 번쩍이거나 수식·동영상·필기·메뉴가
    지연 노출되는 상태
  - 사용자의 스크롤을 되돌리거나, 여러 번 터치해야 반응하거나, 화면 전환 뒤
    포커스·입력값·위치가 예측과 다르게 바뀌는 상태
- 일반적인 터치 대상은 가능한 한 Android 권장 최소 크기 수준을 확보하되,
  크기 확대 때문에 콘텐츠를 가리거나 화면 균형을 깨지 않도록 한다. 정렬,
  간격, 글자 크기, 대비와 버튼 우선순위는 같은 화면 안에서 일관돼야 한다.
- 로그인 뒤 표시 이름이 정확히 `테스트`인지 확인한다. 다르면 문제 화면으로
  진행하지 말고 안전 종료하며 `TEST_ACCOUNT_MISMATCH`로 기록한다.

### 사용자 개입 처리

- USB 재승인, Windows 잠금·인증, 실물 카드 위치·조명·반사, 프린터·종이,
  케이블·전원, 물리 회전·거치와 같이 실제 손이 필요한 항목은 `사용자 개입
  대기`로 기록한다.
- 한 항목에 사용자 개입이 필요하다는 이유로 Goal 전체를 멈추지 않는다.
  우회가 결과를 왜곡하지 않는 안전한 자동시험·합성시험·다른 화면의 실기·
  코드 검토·문서화를 계속한다.
- 우회가 검증을 가장할 수 있는 경우에는 통과로 보고하지 않고 광학·실물·
  물리 검증을 별도 미검증으로 남긴다.
- 모든 다른 의미 있는 작업을 소진했거나 사용자 선택이 없으면 서로 다른
  제품 결과 중 하나를 임의로 확정해야 할 때만 사용자에게 요청하고 대기한다.

### 계속 금지되는 작업

- 공장초기화, Device Owner 제거·교체, 앱 데이터·DB·Keystore·관리자 PIN
  초기화, Kiosk/Web POC 삭제
- A 또는 PC 재부팅·종료, USB 디버깅·ADB 승인·Wi-Fi·잠금화면·접근통제 변경
- 실제 프린터 작업 전송, QR 재발급·폐기, 기존 실물 QR 무효화
- 사용자 손 없이는 회복할 수 없는 고의 잠금, 운영 데이터 파괴와 복구 불능
  실패주입
- Android Emulator·Hyper-V·가상화 기반 계측 환경 시작 또는 활성화. 이전
  PC 비정상 재시작 위험 때문에 A의 안전한 시험과 JVM·정적 시험을 사용한다.
- 관리자 PIN·QR 원문·학생 자격정보를 콘솔, Git, 로그, 문서나 화면 캡처에
  의도적으로 남기는 작업

같은 signer의 더 높은 versionCode APK를 `adb install -r`로 설치하는 것은
기존 데이터·UID·firstInstallTime·Device Owner를 보존하고 설치 전후 상태와
해시를 검증하는 조건에서 허용한다. 업데이트 뒤 실패폐쇄 복구에 PIN이
필요하면 저장 PIN 전용 도구를 사용할 수 있다.

### Goal 수명과 전달

- Goal 입력란에는 `docs/CONTINUOUS_DEVELOPMENT_GOAL_PROMPT.md`의 본문을
  사용한다. 세부 기준은 이 헌장에 두고 호출 프롬프트는 짧은 참조로 유지한다.
- 이 Goal은 사용자가 명시적으로 중단할 때까지 완료 처리하지 않는다.
- 알려진 결함이 잠시 소진돼도 코드·실제 화면·경계 상태·복구·성능·운영
  문서의 다음 고가치 검토 대상을 선택해 사이클을 계속한다.
- 시간 채우기용 변경, 근거 없는 디자인 재작성과 무의미한 버전 상승은 하지
  않는다. 변경할 가치가 없으면 검토 증거를 남기고 다음 범위로 이동한다.
- 한 가지 목적 또는 밀접한 결함 묶음마다 자동시험과 가능한 A 실기를 완료한
  뒤 작은 커밋을 만들고 작업 브랜치에 푸시한다.
- 사용자 기존 미커밋 변경과 `diagnostics/`, `output/`, `tmp/`의 사용자·기존
  산출물은 현재 작업 변경으로 간주하거나 커밋하지 않는다.
- 수행하지 않은 실기와 검증은 완료로 보고하지 않는다. 최종 인계에는 변경,
  화면 실기, 자동시험, A 설치 상태, 커밋·푸시, 롤백 단위, 미검증과 사용자
  개입 대기를 사람이 이해할 수 있게 정리한다.
...
않는다. 코드·시험·실물·운영 문서를 요구사항별로 감사한다. 현재 무인 Goal은
사용자가 명시적으로 중단할 때까지 완료 처리하지 않는다.
...
아래 내용은 연속 개발 초기의 설치·검증 기록이며 현재 설치 기준이 아니다.
현재 기준은 문서 최상단의 `현재 최우선 무인 운영 기준`을 사용한다.
...
- 자격정보, 관리자 PIN, QR 원문, 답안과 학생 실명을 파일·Git·로그에 남기지
  않는다.
- 원문 UI dump, 트래픽 복호화, 루팅과 인증 우회는 사용하지 않는다. 원격
  조작은 최신 화면·상태를 확인한 시험 흐름에서만 좌표 입력을 허용하며,
  오래된 좌표의 맹목적 재사용이나 운영용 고정 좌표 자동화는 사용하지 않는다.
...
2026-08-01 이후에는 저장 PIN 전용 입력, 만료형 화면 캡처, 시험 QR hash
제출과 최신 화면에 근거한 ADB UI 입력을 허용한다. 구체적 조건은 문서
최상단의 최신 무인 운영 기준을 따른다.
...
2. 사용자 물리 조작이나 새로운 고위험 승인이 없이는 남은 모든 의미 있는
   진행이 불가능하다.
...
이 절은 2026-07-28 당시의 동석 운영 기록이다. 2026-08-01 이후에는 문서
최상단의 `현재 최우선 무인 운영 기준`이 이 절보다 우선한다. 작업을 시작하거나
자동 재개할 때마다 이 파일 전체와 사용자 제공 `AGENTS.md`, 현재 Git 상태를
먼저 읽는다.
...
- 사용자 조작·관찰이 필요한 항목은 미검증과 요청 목록에 남기고, 결과를
  왜곡하지 않는 다른 실기·자동시험·검토를 계속한다. 사용자 선택 없이는
  제품 결과가 달라지는 판단만 임의로 확정하지 않는다.
...
- 미검증 항목은 미검증으로 남기며, 사용자 응답을 기다리는 상태를 완료로
  보고하지 않는다.
- 최초 요구사항과 후속 요구를 요구사항별로 검증하고 전방위 감사를 반복한다.
  알려진 실행 가능한 중요 문제가 잠시 남지 않아도 사용자가 중단하기 전에는
  Goal을 완료 처리하지 않는다.
...
- PC 재부팅 자동 시작 실기:
  - PC 부팅 `2026-07-29 11:50:42` 뒤 수신기가 `11:52:55`부터 자동 실행
  - 설치 경로의 수신 프로세스와 TCP 48129 `LISTEN` 확인
  - 수신 EXE 설치본과 보관본 SHA-256 계속 일치
  - 같은 시점 A 전용 HOME·`LOCKED`·`QR_READY`, 복구 요구와 fatal 없음
- 사용자가 직접 프린터 수정을 필수로 하지 않기로 결정했으므로 Android
  직접 인쇄는 현재 운영 범위에서 보류하고 지정 PC 전송을 기본 경로로 삼는다.

## Kiosk RC39 QR 대기 화면 정리 — 2026-07-29

- QR 대기 화면의 상단 제목·잠금 상태·중복 안내를 제거하고, 위치 조정 안내를
  중앙 메시지 영역으로 통합했다.
- 카메라 전환과 관리자 진입은 우측 하단의 카메라·자물쇠 아이콘으로 변경했다.
- 중앙 안내는 실제 활성 렌즈에 맞춰 다음처럼 즉시 갱신한다.
  - `QR 카드를 전면 카메라 렌즈에 보여주세요`
  - `QR 카드를 후면 카메라 렌즈에 보여주세요`
- PC 페어링 QR 안내도 실제 활성 전면·후면 렌즈를 표시한다. 카메라 영상
  미리보기는 개인정보 노출과 화면 복잡도를 늘리므로 추가하지 않았다.
- 구현 커밋:
  - `9b444ee` QR 대기 화면 단순화
  - `a5ae771` 실제 활성 카메라 안내
  - `434c1c0` Kiosk RC39 준비
- 자동 검증:
  - Kiosk 단위시험·debug APK·계측시험 소스 컴파일·lint: 통과
  - Android 13 에뮬레이터 전체 계측 39개: 실패·오류 0
  - release 158 tasks와 APK 이중 검증: 통과
- A 보존형 설치:
  ...
  - UID `10288`, firstInstallTime `2026-07-24 12:52:28`, dataDir,
    카메라 권한, Device Owner와 전용 HOME 유지
  - 설치 뒤 Kiosk 작업을 정상 재생성해 최종 `LOCKED` 복구
  - 재시작 뒤 Matholic 관련 fatal 일치 항목 0
- 사용자 실물 확인:
  - 복구 후 QR 대기 화면 진입: 통과
  - 전면·후면 카메라별 중앙 안내 전환: 통과
  - 상단 중복 요소 제거, 중앙 위치 안내, 우측 하단 아이콘: 통과
- 실기 완료 뒤 자동시험용 Android 에뮬레이터는 종료했다.

## Web POC RC47 제출 연타와 복구 교착 방지 — 2026-07-30

- 전체답안 자동 이동 직후 같은 위치를 연속 터치하면 새로 노출된 최종 제출로
  입력이 관통할 수 있던 경로를 1.5초 재진입 차단으로 막았다.
- JavaScript 확인창은 열린 뒤 0.8초 동안 확인·취소를 잠그고 바깥 터치로
  닫히지 않게 했다.
- 결과 수집 차폐 중 WebView가 멎어도 네이티브 `채점 끝내기`만 동작하던
  장애에서, 로그아웃과 관리자 복구가 같은 멎은 렌더러를 계속 재사용하던
  결함을 수정했다.
- 렌더러 무응답 또는 복구 시간 초과 때 기존 렌더러를 폐기하고 새 WebView로
  복구를 한 번 재시도한다. 일반 학생 세션의 무응답은 기존 실패폐쇄 잠금을
  유지한다.
- Web POC `0.4.0-rc47`/code 64를 A에 보존형 설치했다.
  - APK SHA-256:
    `A1AB886B7F372AA5DFEC84B1D009F73B89C4B074D43A4640F47E3CBB325B1ED2`
  - 기존 데이터, credential bridge 권한, Kiosk Device Owner·전용 HOME 유지
- 검증:
  - Web POC JVM 단위시험·debug assemble·lint 통과
  - Android 13 DOM 계약 44개, 복구 35개 모두 실패 0
  - release 158 tasks와 APK 이중 검증 통과
- 실제 장애 당시 터치 좌표와 렌더러 stack이 남아 있지 않아 최초 불필요
  터치의 정확한 좌표까지는 확정하지 못했다. 제출 관통과 동일 렌더러 복구
  결함은 재현·회귀시험으로 확인했다.
- 실제 학생 계정 제출→결과→안전 종료는 사용자가 태블릿에서 한 번 더
  종단간 확인한다.

## Web POC RC48 주관식 입력 터치 복구 — 2026-07-30

- 주관식 수식 편집기 준비 실패 때 답안 scope 전체에
  `pointer-events: none !important`를 적용하던 것이 직접 원인이었다.
  실제 사이트의 비동기 DOM 순서에서 준비 검사가 실패하면 입력창과
  루트·분수·파이까지 모두 터치 불가로 남았다.
- 수정:
  - 준비 실패 시 전체 scope 입력 차단 제거
  - RC47이 남긴 pointer/ARIA pending 상태 즉시 복구
  - 편집기 준비 전에는 기본 입력과 입력기 메뉴를 fallback으로 유지
  - 편집기 뒤 모든 형제 버튼을 숨기던 과도한 CSS 제거
  - 수식 편집기 최소 터치 높이 56px, 폭 220px, 글자 22px 보장
  - scope 밖에 지연 생성된 toolbar도 방향 키패드와 연결
- Web POC `0.4.0-rc48`/code 65를 A에 보존형 설치했다.
  - APK SHA-256:
    `939351DAA3F6E6EC822000CD704CD489A09700BF5613068776E9F19E706FA9AF`
  - 기존 앱 데이터, credential bridge 권한, Device Owner·전용 HOME 유지
- 검증:
  - JVM 단위시험·debug assemble·lint 통과
  - Android 13 DOM 계약 45개, 복구 35개 전부 통과
  - release 158 tasks와 APK 이중 검증 통과
- 실제 사이트 주관식 입력과 제출·안전 종료는 사용자 실물 재확인 대기다.

## Web POC RC50 회색 지우기 버튼과 방향 키패드 조건 — 2026-07-30

- RC48의 주관식 터치 불가와 제출 장애는 사용자 확인에서 재현되지 않았다.
- 숫자 입력 뒤의 회색 지우기 버튼은 수식 편집기 인접 형제 범위에서만
  식별해 숨긴다. 루트·분수·파이와 입력기·제출 control은 제외한다.
- 방향 키패드는 수식 편집기 존재만으로 표시하지 않는다.
  - 입력란 직접 pointerdown/click/focusin: 표시
  - 입력란·키패드 밖 또는 답안제출 pointerdown: 즉시 숨김
- Web POC `0.4.0-rc50`/code 67을 A에 보존형 설치했다.
  - APK SHA-256:
    `DC5BBAD1F94201BC740BBA72283874D131931F80351F433179981654D0794EFF`
  - 기존 데이터, credential bridge 권한, Device Owner·전용 HOME 유지
- 검증:
  - JVM 단위시험·debug assemble·lint 통과
  - Android 13 DOM 계약 46개 전부 통과
  - release 158 tasks와 APK 이중 검증 통과
...
## Web POC RC56 기존 임시답안이 있는 실패 편집기 복구 — 2026-07-30

- RC55 실기에서 입력칸 크기는 정상화됐지만 문제 화면에 보이는 임시답안이
  답안제출창에는 보이지 않고 입력도 되지 않는 현상을 확인했다.
- 제출창의 React 수식 컴포넌트는 mount됐으나 MathQuill ref만 `null`인
  상태이므로 runtime 선로딩만으로는 이미 실패한 컴포넌트를 복구할 수 없다.
- 실패한 초기 `span`의 React Fiber에서 현재 `latex`와 `onLatexChange`를
  찾아 새 MathField를 원래 답안 상태에 직접 연결한다.
- 기존 임시답안을 새 MathField에 복원하고 이후 삭제·입력을 원래 React
  변경 콜백으로 전달한다.
- 원래 ref에 의존하던 루트·분수·파이도 복구 MathField에 다시 연결한다.
- 정상 수식 편집기, 숨겨진 입력기와 비수식 문제는 변경하지 않는다.
- Web POC 버전은 `0.4.0-rc56`/code 73,
  Web 계약은 `web-2026-07-30.8`이다.
- 검증:
  - 기존 답안 28 표시→삭제 시 null→루트 새 입력 반영 회귀시험 통과
  - Android 13 DOM 계약 52개 전부 통과
  - Android 13 전체 계측시험 89개 전부 통과
  - release 단위시험·lint·두 APK assemble 158 tasks와 APK 이중 검증 통과
- A에 보존형 설치했고 artifact와 설치 APK SHA-256은
...
## Web POC RC58 정상 초기 렌더와 지속 실패 분리 — 2026-07-30

- RC57은 답안창 직후 정상 MathQuill effect가 실행되기 전의 `span`을
  실패로 오인해 여러 입력기를 동시에 재마운트했다.
- 최초 관찰 뒤 1.5초 동안은 재마운트하지 않는다.
- 유예 중 `.mq-editable-field`가 생성된 정상 입력기는 그대로 유지한다.
- 1.5초 이상 class 없이 남은 표시 중인 개별 입력기만 기존 값을 보존한
  공식 `EQ → ONE → EQ` 재마운트 대상으로 삼는다.
- Web POC 버전은 `0.4.0-rc58`/code 75,
  Web 계약은 `web-2026-07-30.10`이다.
- 검증:
  - 답안창 초기 상태 재마운트 0회
  - 1.5초 지속 실패 상태 재마운트 1회
  - 기존 28, 수정 35와 빈 값 삭제의 임시저장 배열 반영 통과
  - Android 13 DOM 계약 52개 전부 통과
  - release 단위시험·lint·두 APK assemble 158 tasks와 APK 이중 검증 통과
- A에 보존형 설치했고 artifact와 설치 APK SHA-256은
...
- S1의 안전 복구, 기존 데이터, 자가진단, CSV, 관리자 실행취소와
  일회용 학생 변경, `릴리스 테스트` 반 Web 검사 후 안전 시작과
  `QR_READY` 검증은 통과했다.
- S1은 완료됐으며 다음 현장 흐름은 S2의 QR 입구 검증이다.
