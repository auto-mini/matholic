...
## 무인 연속 개발 Goal 재개 — 2026-07-28

이 절은 위의 2026-07-26 종료 기록보다 최신 사용자 지시이며, 사용자가 다시
명시적으로 중단할 때까지 연속 개발 Goal을 재개한다. 작업을 시작하거나 자동
재개할 때마다 이 파일 전체와 사용자 제공 `AGENTS.md`, 현재 Git 상태를 먼저
읽고 이 최신 절을 현재 운영 기준으로 적용한다.
...
- 앱 업데이트 실패폐쇄 복구와 RC33의 0.9초 이름 표시 실물 재검증은 사용자
  조작 대기
...
- 기존 “사용자 부재 중 안전 경계”의 금지 항목을 그대로 지킨다.
...
- 사용자 도움이 반드시 필요해져도 그 전까지 가능한 비대면 조사·구현·시험·
  문서화·커밋·푸시를 모두 끝낸 뒤, 필요한 조작을 중복 없이 한 번에 정리한다.
- 명백한 버그, 보안·복구·데이터 무결성 결함, 이미 확정된 사용자 요구의
  누락은 합리적으로 판단해 구현한다. 반면 서로 다른 운영 결과를 만드는
  주관적 UI·업무 흐름·데이터 정책 선택은 한 방향을 몰래 확정하지 않는다.
  가능한 범위에서 근거·대안·권장안을 준비하고 독립적인 다른 작업을 계속한다.
- 미검증 항목은 미검증으로 남기고, 사용자 응답을 기다리는 동안에도 독립적인
  안전 작업이 있으면 계속 진행한다.
- 사용자가 명시적으로 중단하기 전에는 이 연속 Goal을 완료 처리하지 않는다.
